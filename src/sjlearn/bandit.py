# Credits to Nathanael Fijalkow (https://github.com/nathanael-fijalkow/Multi-Armed-Bandits)
# Heavily inspired from Jaromir Janisch (https://github.com/jaromiru)

from typing import List, Literal, Optional, cast, override
import matplotlib
matplotlib.use('Agg')
...
class Bandit:
    def __init__(self, k_arm=10,
                 strategy: Literal['epsilon-greedy', 'UCB'] = 'epsilon-greedy',
                 update_mode = 'average', epsilon = 0.1, UCB = 0.4,
                 step_size = 0.1, initial = 0):
    ...
    def choose_action(self) -> BanditAction:
        if self.strategy == 'epsilon-greedy':
            ...
            return cast(int, np.random.choice([action for action, q in enumerate(self.q_estimation) if q == q_best]))
        if self.strategy == 'UCB':
            # using the UCB algorithm
            if np.all(self.action_count != 0):
                UCB_estimation: np.ndarray[tuple[int], np.dtype[np.float64]] = self.q_estimation + self.UCB * np.sqrt(np.log(self.time + 1) / self.action_count)
                q_best = np.max(UCB_estimation).item()
                return np.random.choice([action for action, q in enumerate(UCB_estimation) if q == q_best])
            else:
                return cast(int, np.random.choice([action for action in np.arange(self.k) if self.action_count[action] == 0]))                
        raise NotImplementedError("Set training strategy not recognized")

    def update(self, action: BanditAction, reward: float):
        # state an action as played with its given reward and update the estimation for this action
        ...
        elif self.update_mode == 'step_size':
            # update with constant step size
            self.q_estimation[action] += self.step_size * (reward - self.q_estimation[action])
        else:
            raise NotImplementedError("Set updating strategy not recognized")
        return reward

class SimulationBandit(Bandit):
    """A bandit agent with the reward that is internally initialised."""
    def __init__(self, k_arm=10, strategy: Literal['epsilon-greedy', 'UCB'] = 'epsilon-greedy', update_mode='average', epsilon=0.1, UCB=0.4, step_size=0.1, initial=0):
        super().__init__(k_arm, strategy, update_mode, epsilon, UCB, step_size,
                         initial)
    @override
    def reset(self):
    ...
    @override
    def update(self, action: BanditAction, reward: Optional[float]=None):
        # generate the reward under using a Gaussian distribution centered around the reward with standard deviation 1
        reward = reward  # unused (declared for a correct overriding)
        used_reward = self.q_true[action] + np.random.randn()
        return super().update(action, used_reward)

def simulate(runs, time, bandits: List[SimulationBandit]):
...
def parameter_epsilon(runs=2000, time=1000):
    ...
    bandits = [SimulationBandit(epsilon = eps) for eps in epsilons]
...
def parameter_UCB(runs=2000, time=1000):
    UCBs = np.arange(0, 1, step = 0.4, dtype=np.floating)
    bandits = [SimulationBandit(strategy = 'UCB', UCB = ucb.item()) for ucb in UCBs]
    best_action_counts,rewards = simulate(runs, time, bandits)
...
def update_step_size(runs=2000, time=1000):
    ...
    bandits = [SimulationBandit(update_mode = 'step_size', step_size = step_size.item()) for step_size in step_sizes]
    best_action_counts, rewards = simulate(runs, time, bandits)
...
def optimistic_evaluation(runs=2000, time=1000):
    epsilons = [0, 0.02, 0.1]
    bandits = [SimulationBandit(epsilon = eps, initial=5) for eps in epsilons]
    best_action_counts, rewards = simulate(runs, time, bandits)
